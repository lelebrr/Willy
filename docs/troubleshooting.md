...
## Novos módulos (pós-unificação)

| Sintoma | Causa / solução |
|---|---|
| `ERROR: RFID module not found` no `rfid` | ST25R/PN532 com pinos errados ou sem alimentação; selecione o módulo em RFID → Config → Modulo RFID |
| Módulo RFID volta para RFID2 sozinho | builds `LITE_VERSION` não incluem ST25R3916 (esperado) |
| Touch/PMIC "morre" até power-cycle | I2C sistêmico travado; o watchdog (`checkAndRecoverSysI2CBus`, 1×/s) recupera sozinho — se persistir, confira pull-ups e `sys_i2c` |
| Heap caindo a cada scan WiFi | corrigido: `WiFi.scanDelete()` após consumo em todos os fluxos (menus, ataques, JS) |
| Cyber Menu abre o menu errado | corrigido: roteamento por nome; atualize o firmware |
| `LV_USE_*` erro de compilação | use só `include/lv_conf.h` (o `lv_conf.h` da raiz foi removido); widgets ociosos vêm desligados |
| `class WebServer` colide no build | removido: `src/web_server.*` e `src/web_files/` deletados (servidor demo órfão) |
| `wdgwarsApiKey` inválida | precisa de 64 hex de `wdgwars.pl/profile` (Config → Avancado → Chave WDGWars) |
| Placa nova sem `SYS_I2C_*` | fallback `-1` em `precompiler_flags.h`; `sys_i2c` fica inválido e o watchdog vira no-op |
