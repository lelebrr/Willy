...
## Novas configurações (pós-unificação)

| Campo | Onde | Descrição |
|---|---|---|
| `wdgwarsApiKey` | Config → Avancado → Chave WDGWars / `settings wdgwarsApiKey <chave>` | 64 hex de wdgwars.pl/profile |
| `ST25R_Pins` | Config → Modo Dev → Pinos ST25R3916 | SPI do ST25R3916 (persistido) |
| `sys_i2c` | automático (placa) | I2C do sistema; somente leitura na prática |
| Layout de teclado | Outros → BadUSB & HID → Layout Teclado | 14 layouts (persiste) |
| `i2c_bus` | Config → Modo Dev → Pinos I2C | barramento do usuário (400 kHz) |

---

## Hostname e credenciais WebUI

| Campo | Onde | Descrição |
|---|---|---|
| `hostname` | Config → Avancado → Nome do Host / `settings hostname <nome>` | usado no DHCP (`WiFi.setHostname`) e no mDNS; padrão `willy` |
| WebUI user/pwd | Config → Avancado → Credenciais WebUI | `setWebUICreds` (antes só pela página de login) |
