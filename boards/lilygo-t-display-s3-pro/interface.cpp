...
void InputHandler(void) {
    ...
    if (millis() - tm > 200 || LongPress) {
        ...
        if (touched && touch.isPressed()) {
            tm = millis();
            if (wilyConfigPins.rotation == 1) { t.y[0] = TFT_WIDTH - t.y[0]; }
            if (wilyConfigPins.rotation == 3) { t.x[0] = TFT_HEIGHT - t.x[0]; }
            ...
            // Serial.printf("\nPressed x=%d , y=%d, rot: %d", t.x[0], t.y[0], wilyConfigPins.rotation);
