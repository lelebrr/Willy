...
void ibeacon(const char *DeviceName, const char *BEACON_UUID, int ManufacturerId) {
    ...
    padprintln("");
    padprintln("Pressione qualquer tecla para PARAR.");
...
void aj_adv(int ble_choice) {
    int count = 0;
    String spamName = "";
    if (ble_choice == 6) { spamName = keyboard("", 10, "Nome para spam"); }

    if (ble_choice == 5) {
        displayTextLine("Spam Todos Seq.");
        padprintln("");
...
void legacySubMenu() {
    ...
    legacyOptions.push_back({"Voltar", []() { returnToMenu = true; }});

    loopOptions(legacyOptions, MENU_TYPE_SUBMENU, "Apple Spam (Legado)");
}
void spamMenu() {
    std::vector<Option> options;

    options.push_back({"Spam Apple", [=]() { appleSubMenu(); }});
    options.push_back({"Spam Apple (Legado)", [=]() { legacySubMenu(); }});
    options.push_back({"Spam Windows", lambdaHelper(aj_adv, 2)});
    options.push_back({"Spam Samsung", lambdaHelper(aj_adv, 3)});
    options.push_back({"Spam Android", lambdaHelper(aj_adv, 4)});
    options.push_back({"Spam Todos", lambdaHelper(aj_adv, 5)});
    options.push_back({"Spam Personalizado", lambdaHelper(aj_adv, 6)});
    options.push_back({"Voltar", []() { returnToMenu = true; }});

    loopOptions(options, MENU_TYPE_SUBMENU, "Spam Bluetooth");
}
