...
buildModelOptions(NimBLERemoteCharacteristic *pTXChar, std::vector<Option> &deviceSelection) {
    ...
    for (size_t i = 0; i < sizeof(payloads) / sizeof(payloads[0]); ++i) {
        ...
        charOptions.push_back({pd.label, [pTXChar, &deviceSelection, &pd](void) {
                                   ...
                                   displayTextLine(success ? "Escrita sucesso!" : "Falha escrita!");
                                   delay(UI_READ_DELAY);
...
void BLENinebot::loop() {
    ...
    while (!check(EscPress)) {
        ...
        if (results.getCount() == 0) {
            displayTextLine("Nenhuma Scooter encontrada. Tente...");
            delay(UI_READ_DELAY);
            pBLEScan->clearResults();
            ...
            deviceSelection.push_back(
                {name, [&, adv](void) mutable {
                     ...
                     if (!pClient->discoverAttributes()) {
                         displayTextLine("Falha Discover.");
                         clientDisconnect();
                         loopOptions(deviceSelection);
                         return;
                     }
                     ...
                     if (svc != nullptr) {
                         ...
                         if (ch != nullptr && (ch->canWrite() || ch->canWriteNoResponse())) {
                             ...
                             displayTextLine("TX nao gravavel");
                         }
                     } else {
                         displayTextLine("Nao eh scooter");
                     }
        ...
        deviceSelection.push_back({"Escanear novam.", [&]() { returnToMenu = false; }});
