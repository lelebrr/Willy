...
void ARPScanner::setup() {
    ...
    if (ip_info.ip.addr == 0 || ip_info.netmask.addr == 0) {
        Serial.println("Ethernet nao tem IP/netmask, abortando scan ARP");
        displayError("Ethernet nao pronta", true);
        return;
    }
...
void ARPScanner::afterScanOptions(const ScanHosts::Host &host) {
...
#ifndef LITE_VERSION
        {"Conectar SSH", lambdaHelper(ssh_setup, host.ip.toString())},
#endif
        {"Deauth Estacao",
         [=]() {
    ...
    // if(sdcardMounted && wilyConfig.devMode) options.push_back({"ARP MITM (WIP)",  [&](){ opt=5;  }});
    loopOptions(options);
    if (opt == 5) {
