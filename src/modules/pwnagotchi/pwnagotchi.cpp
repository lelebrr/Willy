...
void wilygotchi_setup() {
    initPwngrid();
    initUi();
    state = STATE_INIT;
    Serial.println("Wilygotchi Initialized");
}

void wilygotchi_update() {
...
void wilygotchi_start() {
    ...
    if (setupSdCard()) {
        isLittleFS = false;
        if (!SD.exists("/WillyPCAP")) SD.mkdir("/WillyPCAP");
        if (!SD.exists("/WillyPCAP/handshakes")) SD.mkdir("/WillyPCAP/handshakes");
        handshakeFs = &SD;
    } else {
        if (!LittleFS.exists("/WillyPCAP")) LittleFS.mkdir("/WillyPCAP");
        if (!LittleFS.exists("/WillyPCAP/handshakes")) LittleFS.mkdir("/WillyPCAP/handshakes");
        isLittleFS = true;
    ...
    wilygotchi_setup(); // Starts the thing
    // Draw footer & header
    drawTopCanvas();
    drawBottomCanvas();
