...
bool Chameleon::connect() {
    ...
    if (!chmUltra.connectToChamelon()) {
        displayError("Erro conexao Chameleon", true);
        return false;
    }
...
void Chameleon::selectMode() {
    ...
    if (_hf_read_uid) {
        ...
        addOptionSetMode("HF Emulacao", HF_EMULATION_MODE);
        addOptionSetMode("HF Salvar arquivo", HF_SAVE_MODE);
    }
    addOptionSetMode("HF Ler", HF_READ_MODE);
    addOptionSetMode("HF Scan", HF_SCAN_MODE);
    addOptionSetMode("HF Carregar arq.", HF_LOAD_MODE);
    addOptionSetMode("HF UID Custom.", HF_CUSTOM_UID_MODE);

    if (_lf_read_uid) {
        addOptionSetMode("LF Clonar UID", LF_CLONE_MODE);
        addOptionSetMode("LF Emulacao", LF_EMULATION_MODE);
        addOptionSetMode("LF Salvar arquivo", LF_SAVE_MODE);
    }
    addOptionSetMode("LF Ler", LF_READ_MODE);
    addOptionSetMode("LF Scan", LF_SCAN_MODE);
    addOptionSetMode("LF Carregar arq.", LF_LOAD_MODE);
    addOptionSetMode("LF UID Custom.", LF_CUSTOM_UID_MODE);

    addOptionSetMode("Scan Completo", FULL_SCAN_MODE);
    addOptionSetMode("Reset de Fabrica", FACTORY_RESET_MODE);
...
void Chameleon::setMode(AppMode mode) {
    ...
    switch (mode) {
        ...
        case LF_CLONE_MODE:
            ...
            break;
        case HF_CLONE_MODE:
            padprintln("Tipo Disp.: " + printableHFUID.piccType);
...
void Chameleon::displayBanner() {
    ...
    switch (currentMode) {
        case BATTERY_INFO_MODE: printSubtitle("INFO BATERIA"); break;
        case FACTORY_RESET_MODE: printSubtitle("RESET FABRICA"); break;
        case FULL_SCAN_MODE: printSubtitle("SCAN COMPLETO"); break;

        case LF_READ_MODE: printSubtitle("LF MODO LEITURA"); break;
        case LF_SCAN_MODE: printSubtitle("LF MODO SCAN"); break;
        case LF_CLONE_MODE: printSubtitle("LF MODO CLONE"); break;
        case LF_CUSTOM_UID_MODE: printSubtitle("LF UID CUSTOM."); break;
        ...
        case HF_SCAN_MODE: printSubtitle("HF MODO SCAN"); break;
        case HF_CLONE_MODE: printSubtitle("HF MODO CLONE"); break;
        case HF_WRITE_MODE: printSubtitle("HF MODO GRAVAR"); break;
        case HF_CUSTOM_UID_MODE: printSubtitle("HF UID CUSTOM."); break;
    ...
    tft.setTextSize(FP);
    padprintln("");
    padprintln("Pressione [OK] para mudar modo.");
    padprintln("");
}
...
void Chameleon::dumpHFCardDetails() {
    ...
    if (!pageReadSuccess) padprintln("[!] Falha ao ler blocos de dados");
}
...
uint8_t Chameleon::selectSlot() {
    ...
    loopOptions(options, MENU_TYPE_SUBMENU, "Definir Slot Emulacao");
...
void Chameleon::factoryReset() {
    ...
    loopOptions(options, MENU_TYPE_SUBMENU, "Proceder com Reset Fabrica?");
...
void Chameleon::emulateLF() {
    ...
    if (chmUltra.cmdEnableSlot(slot, chmUltra.RFID_LF) && chmUltra.cmdChangeActiveSlot(slot) &&
        ...
        displaySuccess("Emulacao com sucesso.");
        delayWithReturn(1000);
    } else {
...
bool Chameleon::readFileLF() {
    ...
    filepath = loopSD(*fs, true, "RFIDLF", "/WillyRFID");
...
bool Chameleon::writeFileLF(String filename) {
    ...
    if (!(*fs).exists("/WillyRFID")) (*fs).mkdir("/WillyRFID");
    if ((*fs).exists("/WillyRFID/" + filename + ".rfidlf")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists("/WillyRFID/" + filename + String(i) + ".rfidlf")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/WillyRFID/" + filename + ".rfidlf", FILE_WRITE);

    if (!file) { return false; }

    file.println("Filetype: Wily RFID 125kHz File");
...
void Chameleon::emulateHF() {
    if (!isMifareClassic(hfTagData.sak)) {
        displayError("Nao implementado para este tipo de tag", true);
        return setMode(BATTERY_INFO_MODE);
    }
...
bool Chameleon::writeFileHF(String filename) {
    ...
    if (!(*fs).exists("/WillyRFID")) (*fs).mkdir("/WillyRFID");
    if ((*fs).exists("/WillyRFID/" + filename + ".rfid")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists("/WillyRFID/" + filename + String(i) + ".rfid")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/WillyRFID/" + filename + ".rfid", FILE_WRITE);
...
void Chameleon::saveScanResult() {
    ...
    file.println("Filetype: Wily RFID Scan Result");
