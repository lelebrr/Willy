    ...
    if (aid.empty()) { // If we can't get AID, we can't read the card
        ...
        if (pdol.empty()) { // No PDOL(for example Mastercard)
            ...
            if (!afl.empty()) {
                ...
                Serial.println("Nao foi possivel obter ID AFL");
            }
        } else {
            if (is_visa(&res)) {
...
void EMVReader::display_emv(EMVCard card) {
    ...
    if (card.parsed) {
        ...
        if (!found) { padprintln("Fornecedor cartao desconh."); }
        if (card.pan != nullptr) {
        ...
        if (card.validto != nullptr) {
            ...
            padprintln("Data de validade desconhecida");
        }
    } else {
...
void EMVReader::save_emv(const char *aid, const char *pan, const char *validfrom, const char *validto) {
    ...
    File file = (*fs).open("/WillyRFID/Scans/" + filename, FILE_WRITE);
    ...
    file.println("Fornecedor: " + String(aid));
    file.println("Numero Cartao Credito: " + String(pan));
    file.println("Valido De: " + String(validfrom));
    file.println("Valido Ateh: " + String(validto));
