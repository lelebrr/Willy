...
#if defined(RFID_SERIAL_RX) && defined(RFID_SERIAL_TX)
#define TXD_PIN RFID_SERIAL_TX
#define RXD_PIN RFID_SERIAL_RX
...
void PN532KillerTools::setup() {
    // Reset Pin states
    if (wilyConfigPins.SDCARD_bus.checkConflict(RXD_PIN) ||
        wilyConfigPins.SDCARD_bus.checkConflict(TXD_PIN)) {
        sdcardSPI.end();
    }
    if (wilyConfigPins.CC1101_bus.checkConflict(RXD_PIN) ||
        wilyConfigPins.CC1101_bus.checkConflict(TXD_PIN) ||
        wilyConfigPins.NRF24_bus.checkConflict(RXD_PIN) ||
        wilyConfigPins.NRF24_bus.checkConflict(TXD_PIN)) {
        CC_NRF_SPI.end();
    }
...
void PN532KillerTools::displayInitialScreen() {
    ...
    String line1 = "Conecte PN532/PN532Killer";
    String line2 = "via porta UART.";
    String line3 = "Pressione OK p/ checar tipo.";
...
void PN532KillerTools::playDeviceDetectedSound() {
    ...
    if (SD.exists("/device_detected.wav")) {
        ...
        if (wilyConfig.theme.boot_sound) {
            playAudioFile(
                wilyConfig.themeFS(), wilyConfig.getThemeItemImg(wilyConfig.theme.paths.boot_sound)
            );
...
void PN532KillerTools::hardwareProbe() {
    ...
    if (!ok) {
        ...
        displayBanner();
        printCenterFootnote("Cheque Conexao PN532/Killer");
        displayError("Falha Wake");
        return;
    }
