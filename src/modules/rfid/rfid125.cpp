...
void RFID125::display_banner() {
    ...
    tft.setTextSize(FP);
    padprintln("");
    padprintln("Pressione [OK] muda modo.");
    padprintln("");
    padprintln("");
}

void RFID125::dump_card_details() { padprintln("Dados Tag: " + _printable_data); }
...
void RFID125::save_file() {
    ...
    data.replace(" ", "");
    String filename = keyboard(data, 30, "Nome arq:");
    ...
    if (write_file(filename)) {
        ...
        displayError("Erro ao gravar arq.");
    }
    delay(1000);
    set_state(READ_MODE);
}
