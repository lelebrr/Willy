...
// Create PN532 object based on board
    ...
    if (!nfc->setPassiveActivationRetries(0xFF)) {
        displayError("Falha config retry!", true);
        delay(500);
        return;
    }
    ...
    if (!nfc->SRIX_init()) {
        displayError("Falha init SRIX!", true);
        return;
    }
...
void SRIXTool::select_state() {
    ...
    if (_dump_valid_from_read) {
        options.emplace_back(" -Salvar dump", [this]() { set_state(SAVE_MODE); });
        options.emplace_back(" -Clonar tag", [this]() { set_state(WRITE_TAG_MODE); });
    }
    ...
    if (_dump_valid_from_load) {
        options.emplace_back(" -Gravar na tag", [this]() { set_state(WRITE_TAG_MODE); });
    }
    options.emplace_back("Info PN532", [this]() { set_state(PN_INFO_MODE); });
...
void SRIXTool::display_banner() {
    ...
    switch (current_state) {
        case READ_TAG_MODE: printSubtitle("MODO LER TAG"); break;
        case WRITE_TAG_MODE: printSubtitle("MODO GRAVAR TAG"); break;
        case READ_UID_MODE: printSubtitle("MODO LER UID"); break;
...
void SRIXTool::show_main_menu() {
    ...
    padprintln("");
#ifdef T_EMBED_1101
    padprintln("- !!! - T-Embed CC1101 detec - !!!");
    padprintln("- !!! - Antena Fraca p/ SRIX - !!!");
    padprintln("");
#endif
    padprintln("Funcoes:");
    padprintln("- Ler/Clonar tag comp (512B)");
    padprintln("- Salvar/Carregar dumps .srix");
    padprintln("- Ler UID 8-byte");
...
void SRIXTool::read_tag() {
    ...
    for (uint8_t b = 0; b < 128; b++) {
        if (!nfc->SRIX_read_block(b, block)) {
            displayError("Falha leitura bloco " + String(b));
            delay(2000);
            set_state(READ_TAG_MODE);
            return;
        }
        ...
        // Progress indicator
        progressHandler(b + 1, 128, "Lendo blocos dados");
    }
...
void SRIXTool::write_tag() {
    if (!_dump_valid_from_read && !_dump_valid_from_load) {
        ...
        displayError("Leia ou carregue dump 1o.");
        delay(2000);
        set_state(IDLE_MODE);
        return;
    }
    ...
    for (uint8_t b = 0; b < 128; b++) {
        ...
        progressHandler(b + 1, 128, "Gravando blocos dados");
    }
    ...
    // Final report
    if (blocks_failed == 0) {
        displaySuccess("Gravacao completa!", true);
        ...
        if (blocks_failed > 10) {
            padprintln("Blocos falhos: " + failed_blocks + "...");
        } else {
            padprintln("Blocos falhos: " + failed_blocks);
        }

    } else {
        displayError("Gravacao falhou!", true);
...
void SRIXTool::show_pn_info() {
    ...
    if (_has_hardware_pins) {
        ...
        padprintln("Modo: I2C-Apenas (Polling)");
    }
...
void SRIXTool::save_file() {
    if (!_dump_valid_from_read) {
        displayError("Sem dados na memoria!");
        displayError("Leia uma tag antes.");
        delay(2000);
        set_state(IDLE_MODE);
        return;
    }
    ...
    String filename = keyboard(uid_str, 30, "Nome arq:");
    filename.trim();
    ...
    if (!(*fs).exists("/WillyRFID/SRIX")) (*fs).mkdir("/WillyRFID/SRIX");

    // Check if the file already exists and add number
    String filepath = "/WillyRFID/SRIX/" + filename;
    ...
    if (!file) {
        displayError("Erro criando arquivo!");
        delay(1500);
        set_state(IDLE_MODE);
        return;
    }
...
void SRIXTool::load_file() {
    ...
    if (!(*fs).exists("/WillyRFID/SRIX")) {
        displayError("Nenhum dump encontrado!");
        delay(1500);
        displayError("Pasta /WillyRFID/SRIX faltando");
        delay(2000);
        set_state(IDLE_MODE);
        return;
    }
    ...
    File dir = (*fs).open("/WillyRFID/SRIX");
    if (!dir || !dir.isDirectory()) {
        displayError("Nao abriu pasta SRIX!");
        delay(1500);
        set_state(IDLE_MODE);
        return;
    }
    ...
    // Show Menu
    display_banner();
    padprintln("Selecione arq p/ carregar:");
    padprintln("");

    options = {};
    for (const String &fname : fileList) {
        options.emplace_back(fname, [this, fname, fs]() { load_file_data(fs, "/WillyRFID/SRIX/" + fname); });
    }
...
void SRIXTool::load_file_data(FS *fs, String filepath) {
    ...
    if (!file) {
        displayError("Nao abriu arquivo!");
        delay(1500);
        set_state(IDLE_MODE);
        return;
    }
    ...
    // Verify that the all 128 blocks have been loaded
    if (blocks_loaded < 128) {
        displayError("Dump incompleto!");
        displayError("Carregou " + String(blocks_loaded) + "/128 blocos");
        delay(2000);
        set_state(IDLE_MODE);
        return;
    }
    ...
    int lastSlash = filename.lastIndexOf('/');
    if (lastSlash >= 0) {
        filename = filename.substring(lastSlash + 1); // Remove "/WillyRFID/SRIX/"
    }
