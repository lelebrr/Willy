...
PN532::PN532(CONNECTION_TYPE connection_type) {
    ...
    if (connection_type == CONNECTION_TYPE::I2C)
        nfc.setInterface(wilyConfigPins.i2c_bus.sda, wilyConfigPins.i2c_bus.scl);
#ifdef M5STICK
...
bool PN532::begin() {
    ...
    Serial.printf("[PN532] I2C bus: SDA=%d, SCL=%d\n",
        (int)wilyConfigPins.i2c_bus.sda, (int)wilyConfigPins.i2c_bus.scl);
    ...
    if (_connection_type == CONNECTION_TYPE::I2C_SPI) {
        ...
        Wire.begin(wilyConfigPins.i2c_bus.sda, wilyConfigPins.i2c_bus.scl);
    }
#else
    Wire.begin(wilyConfigPins.i2c_bus.sda, wilyConfigPins.i2c_bus.scl);
#endif
    ...
    bool i2c_check = true;
    if (_use_i2c) {
        // Scan full I2C bus for any devices
        Serial.println("[PN532] Scanning I2C bus...");
        bool anyFound = false;
        for (uint8_t addr = 1; addr <= 0x7F; addr++) {
            Wire.beginTransmission(addr);
            if (Wire.endTransmission() == 0) {
                Serial.printf("[PN532]   Found device at 0x%02X\n", addr);
                anyFound = true;
            }
        }
        if (!anyFound) {
            Serial.println("[PN532]   No I2C devices found on bus!");
        }

        // Check PN532 specific address
        Serial.printf("[PN532] Checking PN532 at address 0x%02X...\n", PN532_I2C_ADDRESS);
        Wire.beginTransmission(PN532_I2C_ADDRESS);
        int error = Wire.endTransmission();
        i2c_check = (error == 0);
        Serial.printf("[PN532] I2C check result: %s (error=%d)\n",
            i2c_check ? "OK" : "FAILED", error);
    }

    nfc.begin();
    Serial.println("[PN532] nfc.begin() done");

    uint32_t versiondata = nfc.getFirmwareVersion();
    Serial.printf("[PN532] Firmware version: 0x%08lX\n", (unsigned long)versiondata);

    bool result = i2c_check || versiondata;
    Serial.printf("[PN532] begin() result: %s\n", result ? "SUCCESS" : "FAILED");
    return result;
}
...
int PN532::save(String filename) {
    ...
    File file = createNewFile(fs, "/WillyRFID", filename + ".rfid");
