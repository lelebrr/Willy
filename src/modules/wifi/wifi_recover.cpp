...
static bool parse_pcap_handshake(FS &fs, const String &path, HandshakeData &hs) {
    ...
    if (!have_m1 && !have_m3) {
        ...
        padprintln("Erro: Precisa M1 ou M3");
        return false;
    }
...
void wifi_crack_handshake(const String &wordlist_path, const String &pcap_path) {
    ...
    FS *fs = nullptr;
    if (!getFsStorage(fs)) {
        displayError("Sem filesystem", true);
        return;
    }

    // Parse handshake
    padprintln("Lendo handshake...");
    HandshakeData hs;
    ...
    padprintln("Handshake carregado!");
    padprintf("SSID: %s\n", hs.ssid[0] ? hs.ssid : "(nao achado)");
    padprintf(
    ...
    if (hs.ssid[0] == '\0') {
        padprintln("SSID nao achado no PCAP");
        String ssid = keyboard("", 32, "Digite SSID:");
        if (ssid.length() == 0) {
            displayError("SSID necessario", true);
            return;
        }
        strncpy(hs.ssid, ssid.c_str(), sizeof(hs.ssid) - 1);
        resetTftDisplay();
    ...
    if (!wf) {
        displayError("Nao abriu wordlist", true);
        return;
    }

    // Start Recovering
    padprintln("Recuperando...");
    padprintln("(Aperte SEL p/ parar)"); // clearer instruction
    padprintln("");
    ...
    if (g_abortRequested) {
        padprintln("");
        padprintln("");
        padprintln("Cancelado pelo usuario");
    ...
    padprintln("");
    padprintln("");
    padprintf("Testadas: %u senhas\n", attempts);
    padprintf("Tempo: %.1f s (%.1f/s)\n", seconds, attempts / (seconds > 0 ? seconds : 1.0));
    padprintln("");
    ...
    if (found) {
        ...
        drawMainBorderWithTitle("Quebra Senha WiFi", true);
        padprintln("");
        ...
        padprintln("Aperte tecla p/ continuar...");
...
void wifi_recover_menu() {
    ...
    drawMainBorderWithTitle("Quebrador WiFi", true);
    padprintln("");
    padprintln("Selecione handshake PCAP...");
