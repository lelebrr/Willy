...
int netcutScanDevices() {
    ...
    if (!iface) {
        displayError("netif WiFi ausente", true);
        return 0;
    }
...
void netcutMenu() {
    ...
    if (!s_gwMacValid) { displayWarning("MAC do gateway ausente!\nPode falhar.", true); }
...
DeviceListMenu:
    ...
    options.push_back({"Re-Escanear", []() { netcutScanDevices(); }});
