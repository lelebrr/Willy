...
void stationDeauth(ScanHosts::Host host) {
    ...
    // Get gateway MAC (using Wily's original function)
    ...
    if (!enhanced_mode) {
        // Fallback to Wily's original AP mode
        ...
        if (!WiFi.softAP(tssid, emptyString, channel, 1, 4, false)) {
            ...
            displayError("Falha iniciar Deauth", true);
            return;
        }
    }
    ...
    // Wily's original display code (keep same structure)
    ...
    padprintln("Modo:" + String(enhanced_mode ? "Melhorado" : "AP"));
    padprintln("");
    padprintln("Aperte tecla p/ PARAR.");
    ...
    while (!check(AnyKeyPress)) {
        ...
        if (enhanced_mode) {
            ...
            // AP mode: Use Wily's original send_raw_frame
