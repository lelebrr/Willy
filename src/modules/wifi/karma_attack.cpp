...
void karma_setup() {
    ...
    displayTextLine("Captura Iniciada");
    tft.setTextSize(FP);
    ...
    for (;;) {
        ...
        if (check(SelPress) || redraw) {
            ...
            if (!redraw) {
                options = {
                    ...
                    {auto_hopping ? "* Troca Auto" : "- Troca Auto",
                     [=]() {
                         auto_hopping = !auto_hopping;
                         displayTextLine(auto_hopping ? "Troca Auto: ON" : "Troca Auto: OFF");
                     }                                                                                              },
                    {hop_interval == FAST_HOP_INTERVAL ? "* Troca Rapida" : "- Troca Rapida",
                     [=]() {
                         hop_interval =
                             (hop_interval == FAST_HOP_INTERVAL) ? DEFAULT_HOP_INTERVAL : FAST_HOP_INTERVAL;
                         displayTextLine(
                             hop_interval == FAST_HOP_INTERVAL ? "Troca Rapida: ON" : "Troca Rapida: OFF"
                         );
                     }                                                                                              },
        ...
        if (currentTime - last_time > 1000) {
            ...
            tft.drawString("Unicos: " + String(uniqueProbes.size()), 10, tftHeight - 18);
            tft.drawCentreString("Pacotes " + String(pkt_counter), tftWidth / 2, tftHeight - 26, 1);
