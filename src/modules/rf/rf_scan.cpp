...
void RFScan::setup() {
    ...
    if (wilyConfigPins.rfScanRange < 0 || wilyConfigPins.rfScanRange > 3) {
        wilyConfigPins.setRfScanRange(3);
    }
    if (wilyConfigPins.rfModule != CC1101_SPI_MODULE) { wilyConfigPins.setRfFxdFreq(1); }

    display_info(received, signals, ReadRAW, codesOnly, autoSave, title);

    if (wilyConfigPins.rfFxdFreq) frequency = wilyConfigPins.rfFreq;
...
void RFScan::RCSwitch_Enable_Receive(RCSwitch rcswitch) {
    if (wilyConfigPins.rfModule == CC1101_SPI_MODULE) {
        rcswitch.enableReceive(wilyConfigPins.CC1101_bus.io0);
    } else {
        rcswitch.enableReceive(wilyConfigPins.rfRx);
    }
}
...
void RFScan::read_rcswitch() {
    ...
    if (decoded) { // if there is a value decoded by RCSwitch, show it
        Serial.println("Sinal RcSwitch capturado");
        blinkLed();
        ++signals;
...
void RFScan::read_raw() {
    ...
    // if there is no value decoded by RCSwitch, but we calculated a CRC, show it
        ...
        Serial.println("Sinal Raw capturado");
        blinkLed();
        ++signals;
        received.preset = "0";
    ...
    // If there is no decoded value and no CRC calculated, only show the data when specified
        ...
        Serial.println("Dados Raw capturados");
        blinkLed();
        ++signals;
        received.preset = "0";
        received.protocol = "RAW";
...
void RFScan::select_menu_option() {
    ...
    if (received.data != "" && received.protocol != "RAW")
        options.emplace_back("Replay como RAW", [this]() { set_option(REPLAY_RAW); });
    ...
    if (wilyConfigPins.rfModule == CC1101_SPI_MODULE && !wilyConfigPins.rfFxdFreq)
    ...
    if (ReadRAW)
        ...
        options.emplace_back("Modo = Decodif", [&]() {
            ReadRAW = true;
            return select_menu_option();
        });
...
void RFScan::set_threshold() {
    options = {
        {"(-55) Mais Preciso", [&]() { rssiThreshold = -55; }},
        {"(-60)",               [&]() { rssiThreshold = -60; }},
        {"(-65) Padrao ",      [&]() { rssiThreshold = -65; }},
        {"(-70)",               [&]() { rssiThreshold = -70; }},
        {"(-75)",               [&]() { rssiThreshold = -75; }},
        {"(-80) Menos Preciso", [&]() { rssiThreshold = -80; }},
    };
    loopOptions(options);
}
void display_info(RfCodes received, int signals, bool ReadRAW, bool codesOnly, bool autoSave, String title) {
    ...
    else padprintln("Gravando: Qualquer sinal RAW.");

    if (autoSave) padprintln("Auto salvar: Ativado");

    if (wilyConfigPins.rfFxdFreq) padprintln("Escaneando: " + String(wilyConfigPins.rfFreq) + " MHz");
    else padprintln("Escaneando: " + String(subghz_frequency_ranges[wilyConfigPins.rfScanRange]));
...
void display_signal_data(RfCodes received) {
    ...
    if (received.preset != "")
        padprintln("Protocolo: " + String(received.protocol) + "(" + received.preset + ")");
    ...
    if (received.key > 0) {
        ...
        if (received.protocol == "RAW") {
            ...
            padprintln("Comprim.: " + String(received.Bit) + " bits");
...
bool RCSwitch_SaveSignal(float frequency, RfCodes codes, bool raw, char *key, bool autoSave) {
    ...
    String subfile_out = "Filetype: Wily SubGhz File\nVersion 1\n";
...
String RCSwitch_Read(float frequency, int max_loops, bool raw) {
    ...
    if (!frequency) frequency = wilyConfigPins.rfFreq; // default from config
...
RestartRec:
    ...
    tft.setCursor(10, 28);
    tft.setTextSize(FP);
    tft.println("Aguardando um " + String(frequency) + " MHz " + "sinal.");
