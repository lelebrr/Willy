...
void ledBlinkExfilChannel() {
    ...
    padprintln("");
    padprintln("Aguardando sincronia...");
    padprintln("Nota: Requer script lado Host");
    padprintln("para piscar Lock LEDs");
    padprintln("ESC para sair.");

    safeUSBInit();

    // In a full implementation, we would register onEvent from USBHIDKeyboard
    // and decode the sequence. Here we simulate the capture demo.
    while (true) {
        InputHandler();
        wakeUpScreen();
        if (check(EscPress) || returnToMenu) return;
...
void hidTimingSideChannel() {
    ...
    padprintln("");
    padprintln("Monitorando Side-channel timing...");
    padprintln("Capturando intervalos...");
    ...
    while (true) {
        ...
        if (millis() - startAt > 2000) {
            String fake_time = String(random(100, 450));
            padprintln("T_DIFF: " + fake_time + "ms => Possivel: " + (char)(random(97, 122)));
            startAt = millis();
        }
        delay(50);
    }
...
void ransomwareSimulationDemo() {
    ...
    padprintln("Simulacao executada.");
    delay(2000);
}
