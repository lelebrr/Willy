...
bool Wigle::_check_token() {
    if (wilyConfig.wigleBasicToken == "") {
        displayError("Token Wigle nao encontrado", true);
        return false;
    }

    auth_header = "Basic " + wilyConfig.wigleBasicToken;
...
void Wigle::dump_wigle_info() {
    display_banner();
    padprintln("Logado no Wigle como " + wigle_user);
    padprintln("");
}
...
bool Wigle::_upload_file(File file, String upload_message) {
    ...
    if (!client.connect(host, 443)) {
        displayError("Falha na conexao API Wigle", true);
        return false;
    }
