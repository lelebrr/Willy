...
bool txIrBuffer(const char *buffer, bool hideDefaultUI) {
    ...
    setup_ir_pin(wilyConfigPins.irTx, OUTPUT);
    // digitalWrite(wilyConfigPins.irTx, LED_ON);
    ...
    digitalWrite(wilyConfigPins.irTx, LED_OFF);
    return true;
}
...
void sendNECCommand(String address, String command, bool hideDefaultUI) {
#ifdef WILLY_IR_SERIAL
    if (!hideDefaultUI) { displayTextLine("Sending (Serial).."); }
    uint16_t addr = strtoul(address.c_str(), nullptr, 16);
    uint8_t cmd = strtoul(command.c_str(), nullptr, 16);
    ysIrtm.sendNEC(addr, cmd);
#else
    IRsend irsend(wilyConfigPins.irTx); // Set the GPIO to be used to sending the message.
    irsend.begin();
    ...
    if (wilyConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= wilyConfigPins.irTxRepeats; i++) { irsend.sendNEC(data, 32); }
    }

    Serial.println(
        "Sent NEC Command" + (wilyConfigPins.irTxRepeats > 0
                                  ? " (1 initial + " + String(wilyConfigPins.irTxRepeats) + " repeats)"
                                  : "")
    );
...
void sendNECextCommand(String address, String command, bool hideDefaultUI) {
    ...
    Serial.println(
        "Sent NECext Command" + (wilyConfigPins.irTxRepeats > 0
                                     ? " (1 initial + " + String(wilyConfigPins.irTxRepeats) + " repeats)"
                                     : "")
    );
    digitalWrite(wilyConfigPins.irTx, LED_OFF);
...
void sendRC5Command(String address, String command, bool hideDefaultUI) {
    IRsend irsend(wilyConfigPins.irTx, true); // Set the GPIO to be used to sending the message.
    irsend.begin();
    if (!hideDefaultUI) { displayTextLine("Sending.."); }
    ...
    if (wilyConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= wilyConfigPins.irTxRepeats; i++) { irsend.sendRC5(data, 13); }
    }
    Serial.println(
        "Sent RC5 Command" + (wilyConfigPins.irTxRepeats > 0
                                  ? " (1 initial + " + String(wilyConfigPins.irTxRepeats) + " repeats)"
                                  : "")
    );
    digitalWrite(wilyConfigPins.irTx, LED_OFF);
...
void sendRC6Command(String address, String command, bool hideDefaultUI) {
    ...
    if (wilyConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= wilyConfigPins.irTxRepeats; i++) { irsend.sendRC6(data, 20); }
    }

    Serial.println(
        "Sent RC6 Command" + (wilyConfigPins.irTxRepeats > 0
                                  ? " (1 initial + " + String(wilyConfigPins.irTxRepeats) + " repeats)"
                                  : "")
    );
    digitalWrite(wilyConfigPins.irTx, LED_OFF);
...
void sendSamsungCommand(String address, String command, bool hideDefaultUI) {
    ...
    if (wilyConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= wilyConfigPins.irTxRepeats; i++) { irsend.sendSAMSUNG(data, 32); }
    }

    Serial.println(
        "Sent Samsung Command" + (wilyConfigPins.irTxRepeats > 0
                                      ? " (1 initial + " + String(wilyConfigPins.irTxRepeats) + " repeats)"
                                      : "")
    );
    digitalWrite(wilyConfigPins.irTx, LED_OFF);
...
void sendSonyCommand(String address, String command, uint8_t nbits, bool hideDefaultUI) {
    ...
    if (wilyConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= wilyConfigPins.irTxRepeats; i++) { irsend.sendSony(data, nbits, 2); }
    }

    Serial.println(
        "Sent Sony Command" + (wilyConfigPins.irTxRepeats > 0
                                   ? " (1 initial + " + String(wilyConfigPins.irTxRepeats) + " repeats)"
                                   : "")
    );
    digitalWrite(wilyConfigPins.irTx, LED_OFF);
...
void sendKaseikyoCommand(String address, String command, bool hideDefaultUI) {
    ...
    if (wilyConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= wilyConfigPins.irTxRepeats; i++) { irsend.sendPanasonic64(msb_data, 48); }
    }

    Serial.println(
        "Sent Kaseikyo Command" + (wilyConfigPins.irTxRepeats > 0
                                       ? " (1 initial + " + String(wilyConfigPins.irTxRepeats) + " repeats)"
                                       : "")
    );
    digitalWrite(wilyConfigPins.irTx, LED_OFF);
...
bool sendDecodedCommand(String protocol, String value, uint8_t bits, bool hideDefaultUI) {
    ...
    if (hasACState(type)) {
        ...
        if (wilyConfigPins.irTxRepeats > 0) {
            for (uint8_t i = 1; i <= wilyConfigPins.irTxRepeats; i++) {
        ...
        if (wilyConfigPins.irTxRepeats > 0) {
            for (uint8_t i = 1; i <= wilyConfigPins.irTxRepeats; i++) { irsend.send(type, value_int, bits); }
        }
    }

    delay(20);
    Serial.println(
        "Sent Decoded Command" + (wilyConfigPins.irTxRepeats > 0
                                      ? " (1 initial + " + String(wilyConfigPins.irTxRepeats) + " repeats)"
                                      : "")
    );
    digitalWrite(wilyConfigPins.irTx, LED_OFF);
...
void sendRawCommand(uint16_t frequency, String rawData, bool hideDefaultUI) {
    ...
    // Enable boost for high power mode
    if (irConfig.mode == IR_MODE_ADVANCED && irConfig.powerMode >= IR_POWER_HIGH) {
        PPM.enableOTG();
    }
#endif

    // Apply advanced configuration: use configured frequency if in advanced mode
    uint16_t txFrequency = frequency;
    if (irConfig.mode == IR_MODE_ADVANCED && frequency == 0) {
        txFrequency = irConfig.getFrequencyHz();
    }
    ...
    // Apply advanced configuration: duty cycle
    if (irConfig.mode == IR_MODE_ADVANCED) {
        irsend.calibrate();
    }
    ...
    while (rawData.length() > 0 && count < dataBufferSize) {
        ...
        // Apply preamble multiplier for longer bursts
        uint16_t pulseValue = dataChunk.toInt();
        if (irConfig.mode == IR_MODE_ADVANCED && irConfig.preambleMode != IR_PREAMBLE_STANDARD) {
            // Apply preamble multiplier to first few pulses
            if (count < 4) {
                pulseValue *= irConfig.getPreambleMultiplier();
            }
        }
        dataBuffer[count++] = pulseValue;
    }

    Serial.println("Parsing raw data complete.");

    // Send raw command
    irsend.sendRaw(dataBuffer, count, txFrequency);

    // Apply advanced configuration: additional repeats from config
    uint8_t totalRepeats = wilyConfigPins.irTxRepeats;
    if (irConfig.mode == IR_MODE_ADVANCED) {
        totalRepeats += irConfig.repeatCount;
    }

    if (totalRepeats > 0) {
        for (uint8_t i = 1; i <= totalRepeats; i++) {
            irsend.sendRaw(dataBuffer, count, txFrequency);
            // Apply inter-command delay from config
            if (irConfig.mode == IR_MODE_ADVANCED && irConfig.interCommandDelay > 0) {
                delay(irConfig.interCommandDelay);
            }
        }
    }
    ...
    Serial.println(
        "Sent Raw Command" + (totalRepeats > 0
                                  ? " (1 initial + " + String(totalRepeats) + " repeats)"
                                  : "")
    );
    digitalWrite(wilyConfigPins.irTx, LED_OFF);
}
