...
void IrRead::loop() {
    ...
    while (1) {
        ...
        if (quickloop && button_pos == (int)quickButtons.size()) save_device();
...
void IrRead::display_btn_options() {
    ...
    tft.println("");
    if (_read_signal) {
        padprintln("Pressione [PREV] p/ descartar");
        padprintln("Pressione [NEXT] p/ salvar");
    }
    if (signals_read > 0) { padprintln("Pressione [OK]   p/ salvar disp"); }
    padprintln("Pressione [ESC]  p/ sair");
}
...
void IrRead::read_signal() {
    ...
    if (!_read_signal && ysIrtm.receiveNEC(&addr, &cmd)) {
        ...
        display_banner();
        padprintln("Capturado (Serial):");
        padprintln("Addr: 0x" + String(addr, HEX));
        padprintln("Cmd:  0x" + String(cmd, HEX));
...
void IrRead::save_device() {
    ...
    if (fs && write_file(filename, fs)) {
        displaySuccess("Arq salvo no " + String((fs == &SD) ? "SD Card" : "LittleFS") + ".", true);
        signals_read = 0;
        strDeviceContent = "";
    } else displayError(fs ? "Erro ao gravar arq." : "Sem armaz. disponivel.", true);
