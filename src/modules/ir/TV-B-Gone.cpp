...
void checkIrTxPin() {
    ...
    int count = 0;
    for (auto pin : pins) {
        if (pin.second == wilyConfigPins.irTx) count++;
    }
    if (count > 0) return;
...
void StartTvBGone() {
    if (!init_ir_tx_mutex()) {
        displayRedStripe("Falha init Mutex");
        delay(2000);
        return;
    }
    ...
    if (!returnToMenu) {
        ...
        for (i = 0; i < num_codes; i++) {
            ...
            if (check(SelPress)) // Pause TV-B-Gone
            {
                ...
                displayTextLine("Rodando, Aguarde");
            }
