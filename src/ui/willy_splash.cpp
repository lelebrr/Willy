...
static lv_obj_t *bubbles[5] = {nullptr};
static lv_obj_t *water_drops[5] = {nullptr};

// Tempos de animação baseados na velocidade
static int getAnimTime(int baseTime) {
    switch(willyLogoConfig.velocidade) {
        case 0: return baseTime * 2;      // Lento
        case 2: return baseTime / 2;      // Rápido
...
static void draw_orca(lv_obj_t *parent) {
    ...
    // Corpo principal da orca (forma oval/elipsóide)
    ...
    lv_obj_set_style_radius(orca_body, 50, 0); // Bem arredondado
    ...
    lv_obj_set_size(orca_dorsal_fin, 15, 35);
    lv_obj_set_pos(orca_dorsal_fin, 45, -25);
    lv_obj_set_style_bg_color(orca_dorsal_fin, lv_color_hex(primary_color), 0);
    lv_obj_set_style_radius(orca_dorsal_fin, 5, 0);
    lv_obj_set_style_transform_angle(orca_dorsal_fin, 450, 0); // Levemente inclinada
    ...
    lv_obj_set_style_opa(willy_label, 0, 0); // Começa invisível
    lv_obj_align(willy_label, LV_ALIGN_CENTER, 0, 50);
    ...
    lv_obj_set_style_opa(version_label, 0, 0); // Começa invisível
    lv_obj_align(version_label, LV_ALIGN_CENTER, 0, 80);
    
    // Criar bolhas
    for(int i = 0; i < 5; i++) {
        bubbles[i] = lv_obj_create(parent);
        lv_obj_set_size(bubbles[i], 4 + (i % 3) * 2, 4 + (i % 3) * 2);
        lv_obj_set_pos(bubbles[i], 50 + i * 15, 80);
        lv_obj_set_style_bg_color(bubbles[i], lv_color_hex(white_color), 0);
        lv_obj_set_style_bg_opa(bubbles[i], LV_OPA_30, 0);
        lv_obj_set_style_radius(bubbles[i], 50, 0);
        lv_obj_set_style_border_width(bubbles[i], 0, 0);
        lv_obj_set_style_pad_all(bubbles[i], 0, 0);
    }
    
    // Criar gotas de água
    for(int i = 0; i < 5; i++) {
        water_drops[i] = lv_obj_create(parent);
        lv_obj_set_size(water_drops[i], 2, 8);
        lv_obj_set_pos(water_drops[i], 80 + i * 10, -20);
        lv_obj_set_style_bg_color(water_drops[i], lv_color_hex(0x44FF), 0); // Azul claro
        lv_obj_set_style_radius(water_drops[i], 50, 0);
        lv_obj_set_style_border_width(water_drops[i], 0, 0);
        lv_obj_set_style_pad_all(water_drops[i], 0, 0);
    }
}

// Inicia todas as animações
static void start_animations(lv_obj_t *parent) {
    int baseTime = getAnimTime(1500);
    
    // Animação 1: Orca vem de longe (translate X)
    lv_anim_t move_x;
    lv_anim_init(&move_x);
    lv_anim_set_var(&move_x, orca_container);
    lv_anim_set_values(&move_x, -150, 0);
    ...
    lv_anim_set_time(&version_fade, getAnimTime(800));
    lv_anim_set_delay(&version_fade, getAnimTime(1200));
    lv_anim_set_exec_cb(&version_fade, (lv_anim_exec_xcb_t)lv_obj_set_style_opa);
    lv_anim_start(&version_fade);
    
    // Animação 6: Bolhas subindo
    for(int i = 0; i < 5; i++) {
        lv_anim_t bubble_up;
        lv_anim_init(&bubble_up);
        lv_anim_set_var(&bubble_up, bubbles[i]);
        lv_anim_set_values(&bubble_up, 80, -20);
        lv_anim_set_time(&bubble_up, getAnimTime(2000 + i * 300));
        lv_anim_set_delay(&bubble_up, getAnimTime(500 + i * 100));
        lv_anim_set_exec_cb(&bubble_up, bubble_anim_cb);
        lv_anim_set_repeat_count(&bubble_up, LV_ANIM_REPEAT_INFINITE);
        lv_anim_set_playback_time(&bubble_up, 0);
        lv_anim_set_ready_cb(&bubble_up, [](lv_anim_t *a) {
            lv_obj_set_y((lv_obj_t*)a->var, 80);
        });
        lv_anim_start(&bubble_up);
    }
    
    // Animação 7: Gotas de água caindo (esguicho)
    for(int i = 0; i < 5; i++) {
        lv_anim_t drop;
        lv_anim_init(&drop);
        lv_anim_set_var(&drop, water_drops[i]);
        lv_anim_set_values(&drop, -20, 100);
        lv_anim_set_time(&drop, getAnimTime(800 + i * 150));
        lv_anim_set_delay(&drop, getAnimTime(300 + i * 50));
        lv_anim_set_exec_cb(&drop, (lv_anim_exec_xcb_t)lv_obj_set_y);
        lv_anim_set_repeat_count(&drop, 3);
        lv_anim_set_ready_cb(&drop, [](lv_anim_t *a) {
            lv_obj_set_y((lv_obj_t*)a->var, -20);
        });
        lv_anim_start(&drop);
    }
}
