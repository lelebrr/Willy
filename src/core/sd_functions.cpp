...
String getHierarchicalPath(String baseDir) {
    ...
    if (!SD.exists(baseDir)) SD.mkdir(baseDir);
        ...
        if (LongPress || SelPress) {
            ...
            if (check(SelPress)) {
                if (fileList[index].folder == true && fileList[index].operation == false) {
                    options = {
                        ...
                        Option{"Deletar",     [=]() { deleteFromSd(fs, Folder + "/" + fileList[index].filename); }},
                    ...
                    if (fileToCopy != "") options.push_back(Option{"Colar", [=]() { pasteFile(fs, Folder); }});
            ...
            Files:
                if (fileList[index].folder == true && fileList[index].operation == false) {
                    ...
                    if (fileToCopy != "") options.push_back(Option{"Colar", [=]() { pasteFile(fs, Folder); }});
                    if (&fs == &SD)
                        options.push_back(Option{"Copiar->LittleFS", [=]() { copyToFs(SD, LittleFS, filepath); }});
