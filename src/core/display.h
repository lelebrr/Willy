 ...
 * @param x: If center=false, x is the X coordinate to start drawing, if
 * center=true, x is the x offset
 * @param y: If center=false, y is the Y coordinate to start drawing, if
 * center=true, y is the y offset
 * @param center: draw the image at the center of the screen
 * @param playDurationMs: time that the GIF will be played
 */
bool drawImg(FS &fs, String filename, int x = 0, int y = 0, bool center = false,
             int playDurationMs = 0, bool resetButtonStatus = true);
bool drawPNG(FS &fs, String filename, int x, int y, bool center);
bool preparePngBin(FS &fs, String filename);
bool drawBmp(FS &fs, String filename, int x = 0, int y = 0,
             bool center = false);
#if !defined(LITE_VERSION)
bool showGif(FS *fs, const char *filename, int x = 0, int y = 0,
             bool center = false, int playDurationMs = 0,
...
void displayRedStripe(String text, uint16_t fgcolor = TFT_WHITE,
...
void displayTextLine(String txt,
                     bool waitKeyPress = false); // UI Colored stripe
...
int loopOptions(std::vector<Option> &options, uint8_t menuType,
                const char *subText, int index = 0, bool interpreter = false);
...
Opt_Coord drawOptions(int index, std::vector<Option> &options, uint16_t fgcolor,
                      uint16_t selcolor, uint16_t bgcolor,
...
void progressHandler(int progress, size_t total,
                     String message = "Executando, aguarde");
...
#define willy_small_height 34
PROGMEM const unsigned char willy_small_bits[] = {
