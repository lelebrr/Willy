...
TwoWire *acquireI2CBus(int8_t sda, int8_t scl) {
    ...
    TwoWire *wire = &Wire;
#endif
    if (userWire != wire || activeSda != sda || activeScl != scl) {
        wire->begin(sda, scl);
        // Fast-mode 400kHz no barramento de usuario: PN532/MFRC522/ST25R/SRIX,
        // gesto e RTC suportam. O sys_i2c compartilhado nao e tocado aqui.
