...
// --- Adaptador SimpleCLI: repassa args posicionais p/ processCommand ---
static void forwardHw(cmd *c, const char *name) {
  Command cmd(c);
  std::vector<String> args;
  for (int i = 0; i < 4; i++) {
    String v = cmd.getArgument("arg" + String(i)).getValue();
    if (v.length() > 0) args.push_back(v);
  }
  HardwareCommands::getInstance().processCommand(name, args);
}

#define HW_CMD(cb, nm)                                                             \
  static uint32_t cb(cmd *c) {                                                     \
    forwardHw(c, nm);                                                              \
    return true;                                                                   \
  }

HW_CMD(hwInfoCb, "hwinfo")
HW_CMD(hwPinInfoCb, "pininfo")
HW_CMD(hwPeripheralCb, "peripheral")
HW_CMD(hwConfigPinCb, "configpin")
HW_CMD(hwTestPinCb, "testpin")
HW_CMD(hwI2CScanCb, "i2cscan")
HW_CMD(hwReportCb, "hwreport")
HW_CMD(hwResetPinsCb, "resetpins")
HW_CMD(hwPeriphStatusCb, "periphstatus")
HW_CMD(hwLoadCfgCb, "loadhwconfig")
HW_CMD(hwSaveCfgCb, "savehwconfig")

void createHardwareCommands(SimpleCLI *cli) {
  Command hw = cli->addCompositeCommand("hw");
  auto add = [&](const char *sub, uint32_t (*cb)(cmd *)) {
    Command s = hw.addCommand(sub, cb);
    s.addPosArg("arg0", "");
    s.addPosArg("arg1", "");
    s.addPosArg("arg2", "");
    s.addPosArg("arg3", "");
  ...
  add("info", hwInfoCb);
  add("pininfo", hwPinInfoCb);
  add("peripheral", hwPeripheralCb);
  add("configpin", hwConfigPinCb);
  add("testpin", hwTestPinCb);
  add("i2cscan", hwI2CScanCb);
  add("hwreport", hwReportCb);
  add("resetpins", hwResetPinsCb);
  add("periphstatus", hwPeriphStatusCb);
  add("loadhwconfig", hwLoadCfgCb);
  add("savehwconfig", hwSaveCfgCb);
}