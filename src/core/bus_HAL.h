...
/*
 * HAL for the user-facing I2C bus (wilyConfigPins.i2c_bus).
 *
 * wilyConfigPins.sys_i2c is the bus used by system peripherals (touchscreen, PMIC, RTC, IMU,
 * ...) set up once in boards/*\/interface.cpp and must never be stopped.
 * wilyConfigPins.i2c_bus is user-configurable at runtime and may end up sharing the same
...
// Tells the HAL which TwoWire instance is physically wired to wilyConfigPins.sys_i2c.
...
// Acquires the TwoWire instance for wilyConfigPins.i2c_bus, starting it on the requested pins
// (defaults to wilyConfigPins.i2c_bus.sda/scl). If those pins match sys_i2c, the shared sys bus
