...
void updateClockTimezone() {
    ...
    timeClient->setTimeOffset(wilyConfig.tmz * 3600);

    localTime = timeClient->getEpochTime() + (wilyConfig.dst ? 3600 : 0);
...
void showDeviceInfo() {
    ...
    area.addLine("Wily Version: " + String(WILLY_VERSION));
    ...
    area.addLine("Brightness: " + String(wilyConfig.bright) + "%");
    area.addLine("");
#endif

    area.addLine("[GPIO]");
    area.addLine("GROVE_SDA: " + String(wilyConfigPins.i2c_bus.sda));
    area.addLine("GROVE_SCL: " + String(wilyConfigPins.i2c_bus.scl));
    area.addLine("SERIAL TX: " + String(wilyConfigPins.uart_bus.tx));
    area.addLine("SERIAL RX: " + String(wilyConfigPins.uart_bus.rx));
