...
public:
    ...
    virtual void drawIconImg() {
        drawImg(
            *wilyConfig.themeFS(),
            wilyConfig.getThemeItemImg(themePath()),
            0,
            imgCenterY,
            true,
            wilyConfig.theme.gifDuration,
            false
        );
    }
    ...
    void draw(float scale = 1) {
        if (rotation != wilyConfigPins.rotation) resetCoordinates();
        if (!checkTheme()) {
            tft.fillRect(0, 27, tftWidth, tftHeight - 27, wilyConfig.bgColor);
            drawIcon(scale);
            drawArrows(scale);
            drawTitle(scale);
        } else {
            if (wilyConfig.theme.label)
                drawTitle(scale); // If using .GIF, labels are draw after complete, which takes some time
            drawIconImg();
            if (wilyConfig.theme.label) drawTitle(scale); // Makes sure to draw over the image
        }
        drawStatusBar();
    }

    void drawArrows(float scale = 1) {
        tft.fillRect(arrowAreaX, iconAreaY, arrowAreaW, iconAreaH, wilyConfig.bgColor);
        tft.fillRect(
            tftWidth - arrowAreaX - arrowAreaW, iconAreaY, arrowAreaW, iconAreaH, wilyConfig.bgColor
        );
    ...
    void drawTitle(float scale = 1) {
        ...
        tft.setTextSize(FM);
        tft.drawPixel(0, 0, 0);
        tft.fillRect(arrowAreaX, titleY, tftWidth - 2 * arrowAreaX, LH * FM, wilyConfig.bgColor);
...
protected:
    ...
    void clearIconArea(void) {
        tft.fillRect(iconAreaX, iconAreaY, iconAreaW, iconAreaH, wilyConfig.bgColor);
    }
    void clearImgArea(void) { tft.fillRect(7, 27, tftWidth - 14, tftHeight - 34, wilyConfig.bgColor); }
    void resetCoordinates(void) {
        ...
        rotation = wilyConfigPins.rotation;
    }
